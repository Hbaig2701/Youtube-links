...
const videos = [
  ...
  { slug: 'passive-income-strategies', title: '5 Passive Income Strategies That Actually Work', youtube_url: 'https://youtube.com/watch?v=ghi789', youtube_video_id: 'ghi789' },
  ...
  { slug: 'retirement-planning-guide', title: 'Complete Retirement Planning Guide', youtube_url: 'https://youtube.com/watch?v=mno345', youtube_video_id: 'mno345' },
];

const insertVideo = db.prepare('INSERT INTO videos (slug, title, youtube_url, youtube_video_id) VALUES (?, ?, ?, ?)');
for (const v of videos) {
  insertVideo.run(v.slug, v.title, v.youtube_url, v.youtube_video_id);
}
...
const linkTemplates = [
  { label: 'book-a-call', destination_url: 'https://calendly.com/example/30min', is_booking_link: 1 },
  { label: 'free-guide', destination_url: 'https://example.com/free-guide', is_booking_link: 0 },
  { label: 'course', destination_url: 'https://example.com/course', is_booking_link: 0 },
  { label: 'newsletter', destination_url: 'https://example.com/newsletter', is_booking_link: 0 },
];

const insertLink = db.prepare('INSERT INTO links (video_id, label, destination_url, is_booking_link) VALUES (?, ?, ?, ?)');
for (const v of videoRows) {
  const numLinks = 2 + Math.floor(Math.random() * 3); // 2-4 links per video
  for (let i = 0; i < numLinks && i < linkTemplates.length; i++) {
    const t = linkTemplates[i];
    insertLink.run(v.id, t.label, t.destination_url, t.is_booking_link);
  }
}

const linkRows = db.prepare('SELECT * FROM links').all();
...
const insertClicks = db.transaction(() => {
  for (let i = 0; i < 200; i++) {
    ...
    insertClick.run(
      link.id,
      clickTime.toISOString().replace('T', ' ').replace('Z', ''),
